...
class AwsV4
{
    ...
    public function getResponse()
    {
        ...
        $fp = @fopen('https://' . $this->host . $this->path, 'rb', false, $stream);
    ...
    {
        $itemResult = json_decode($this->response);
    ...
    {
        $itemResult = json_decode($this->response);
        var_dump(str_replace(".", "", $itemResult->ItemsResult->Items[0]->Offers->Listings[0]->Price->Amount));

        $i = $itemResult->ItemsResult->Items[0];

        $item = new STDclass;
        $item->detailedPageURL = $i->DetailPageURL;
        $item->link = "https://www.amazon.com/gp/product/" . $this->itemID . "/?tag=" . $this->partnerTag;
        ...
        $item->price = $this->get_item_price();
